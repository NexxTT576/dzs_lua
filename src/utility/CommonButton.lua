...
function CommonButton:ctor(params)
    ...
    local boundBox = self.sprite:getBoundingBox()
    ...
    addNodeEventListener(
        ...
        function(event)
            GameAudio.playSound(ResMgr.getSFX(SFX_NAME.u_queding))
            if cc.rectContainsPoint(boundBox, self.sprite:convertToNodeSpace(event:getLocation())) then
                self:onButtonDown()
                return true
            else
                return false
    ...
    addNodeEventListener(
        ...
        function(event)
            local taped = cc.rectContainsPoint(boundBox, self.sprite:convertToNodeSpace(event:getLocation()))
            self:onButtonUp(taped)
        end
    )
