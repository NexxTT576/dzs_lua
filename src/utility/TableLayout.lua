...
local TableLayout =
    ...
    "TableLayout",
    function()
        --    return display.newColorLayer(ccc4(255, 0, 0, 170))
...
function TableLayout:ctor(param)
    ...
    self:setContentSize(cc.size(self._width, self._height))
end
...
function TableLayout:addChildEx(child)
    ...
    if h > self._height then
        self._height = h
        self:setContentSize(cc.size(self._width, self._height + 5))
    end
    child:setScale(0)

    child:setPosition(cc.p(posX, -h + self._nodes[1 + (row - 1) * self._rowNum]:getContentSize().height / 2))
