...
function getRichText(htmlText, lineWidth, hrefHandler)
    function createLabel(props, node, x, y, a)
        local label = newTTFLabel(props):addTo(node):move(x, y)
    ...
    -- 默认的字体属性
    local fontProps = {x = 0, y = 0, size = display.DEFAULT_TTF_FONT_SIZE, font = display.DEFAULT_TTF_FONT, align = cc.TEXT_ALIGNMENT_LEFT}
    -- a属性
    local a = {}
    local label = nil
    ...
    for i, v in ipairs(nodeList) do
        if v.type == nodeType.startTag then
            local props = v.props
            if v.tag == "font" then
                fontProps.size = display.DEFAULT_TTF_FONT_SIZE
    ...
    --    layer:setContentSize(cc.size(lineWidth, totalHeight))
