...
function MsgBox:ctor(param)
    local size = param.size or cc.size(display.width * 0.9, display.height * 0.5)
    ...
    local rootnode = CCBReaderLoad("public/window_msgBox", rootProxy, self._rootnode, baseNode, size)
    ...
    if leftBtnName == "" then
        ...
        self._rootnode["leftBtn"]:registerControlEventHandler(
            function(eventName, sender)
    ...
    if midBtnName == "" then
        ...
        self._rootnode["midBtn"]:registerControlEventHandler(
            function(eventName, sender)
                GameAudio.playSound(ResMgr.getSFX(SFX_NAME.u_queding))
    ...
    if rightBtnName == "" then
        ...
        self._rootnode["rightBtn"]:registerControlEventHandler(
            function(eventName, sender)
                GameAudio.playSound(ResMgr.getSFX(SFX_NAME.u_queding))
                rightBtnFunc()
                self:removeSelf()
