...
function Player:initBaseInfo(param)
    ...
    local function loadStorge()
        local uid = cc.UserDefault:getInstance():getStringForKey("accid")
        if (uid == nil or uid == "") then
            uid = os.time()
            cc.UserDefault:getInstance():setStringForKey("accid", uid)
...
function Player:deleteUID()
    -- self.m_uid = nil
    cc.UserDefault:getInstance():setStringForKey("accid", "")
    cc.UserDefault:getInstance():flush()
end
