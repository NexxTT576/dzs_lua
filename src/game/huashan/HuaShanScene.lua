    ...
    self._rootnode["touchNode"]:addNodeEventListener(
        ...
        function(event, x, y)
            ...
            elseif "moved" == event.name then
                ...
                if posY + event:getLocation().y - event:getPreviousLocation().y > 0 then
                    self._rootnode["imageBg"]:setPosition(posX, 0)
                elseif posY + event:getLocation().y - event:getPreviousLocation().y < self._maxPosY then
                    self._rootnode["imageBg"]:setPosition(posX, self._maxPosY)
                else
                    self._rootnode["imageBg"]:setPosition(posX, posY + event:getLocation().y - event:getPreviousLocation().y)
                end
            end
        end,
        1
...
function HuaShanScene:refreshHero()
    ...
    for k, v in ipairs(self._enemies) do
        local hero =
            require("game.huashan.PosHero").new(
            {
                index = k,
                info = v,
    ...
    for i = 1, 15 do
        ...
        self._rootnode[string.format("box%d", i)]:addNodeEventListener(
            cc.NODE_TOUCH_EVENT,
            function(event)
                if event.name == "began" then
                    self._rootnode[string.format("box%d", i)]:setTouchEnabled(false)

                    self._rootnode[string.format("box%d", i)]:schedule(
                        function()
                            self._rootnode[string.format("box%d", i)]:setTouchEnabled(true)
                        end,
                        2
...
function HuaShanScene:chooseEnemy(index)
    ...
    if (self._floor > 0 and index > self._floor + 1) or (index > 1 and self._floor == -1) then
        ...
        if index == 1 or self._awards[index - 1] then
            local formLayer =
                require("game.huashan.HuaShanFormLayer").new(
                {
...
function HuaShanScene:onReset()
    ...
    if self._resetTimes > 0 then
        local layer =
            require("game.huashan.HuaShanResetTip").new(
            {
                showType = 0,
                listener = function()
