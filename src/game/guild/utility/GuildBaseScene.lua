...
function GuildBaseScene:ctor(param)
    ...
    local node = CCBReaderLoad("guild/guild_window_scene", proxy, self._rootnode)
    ...
    if _topFile then
        ...
        local bottomNode = CCBReaderLoad(_topFile, proxy, self._rootnode)
    ...
    local subTopNode
    if _subTopFile then
        subTopNode = CCBReaderLoad(_subTopFile, proxy, self._rootnode)
    ...
    if _contentFile then
        ...
        local contentNode
        contentNode = CCBReaderLoad(_contentFile, proxy, self._rootnode, self, cc.size(display.width + _adjustSize.width, h + _adjustSize.height))
