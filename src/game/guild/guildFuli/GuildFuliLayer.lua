            ...
            callback = function(data)
                ...
                if data.err ~= "" then
                    ...
                    if fuliType == GUILD_FULIITEM_TYPE.barbecue then
                        ...
                        msgBox =
                            require("game.guild.guildFuli.GuildFuliRewardMsgBox").new(
                            {
                                naili = naili,
                                tili = tili
                            }
...
function GuildFuliLayer:ctor(data)
    ...
    local node = CCBReaderLoad("guild/guild_guildFuli_bg.ccbi", proxy, self._rootnode, self, cc.size(display.width, display.height * 0.94))
