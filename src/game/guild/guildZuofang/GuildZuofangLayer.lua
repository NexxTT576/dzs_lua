...
function GuildZuofangLayer:createAllLbl()
    ...
    -- 普通生产 得到的道具
    self._normalAddLbl = self:createShadowLbl("待定", cc.c3b(0, 219, 52), self._rootnode["normal_add_lbl"])
    self._normalAddLbl:setPosition(-self._normalAddLbl:getContentSize().width / 2, self._normalAddLbl:getContentSize().height / 2)

    -- 加班生产 得到的道具
    self._fastAddLbl = self:createShadowLbl("待定", cc.c3b(0, 219, 52), self._rootnode["fast_add_lbl"])
