...
local GuildDadianContributeItem =
    class(
    "GuildDadianContributeItem",
    function()
...
function GuildDadianContributeItem:create(param)
    ...
    msgLbl = self:createTTF("个人贡献", cc.c3b(0, 219, 52), self._rootnode["msg_lbl_3_3"])
    msgLbl:setPosition(-msgLbl:getContentSize().width, 0)
...
function GuildDadianContributeItem:refreshItem(itemData)
    ...
    self._rootnode["coin_icon"]:setSpriteFrame(display.newSprite(iconName):getSpriteFrame())
    local titleName = "#guild_dd_title_icon_" .. tostring(self._id) .. ".png"
    self._rootnode["title_icon"]:setSpriteFrame(display.newSprite(titleName):getSpriteFrame())
