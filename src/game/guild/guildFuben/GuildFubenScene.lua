    ...
    RequestHelper.Guild.showHurtList(
        {
            ...
            callback = function(data)
                ...
                if data.err ~= "" then
                    ...
                    local layer =
                        require("game.guild.guildFuben.GuildFubenRankLayer").new(
                        {
                            hurtList = hurtList,
...
function GuildFubenScene:ctor(param)
    ...
    helpBtn:registerControlEventHandler(
        function(eventName, sender)
            ...
            GameAudio.playSound(ResMgr.getSFX(SFX_NAME.u_queding))
            local helpLayer =
                require("game.guild.guildFuben.GuildFubenHelpLayer").new(
                {
...
function GuildFubenScene:reloadListView(listData)
    ...
    local function showRewardBox(itemData, cell)
        -- 显示奖励信息
        local msgBox =
            require("game.guild.guildFuben.GuildFubenRewardMsgBox").new(
            {
        ...
        game.player:getGuildMgr():RequestFubenInfo(
            {
                ...
                cb = function(rtnObj)
                    ...
                    local infoLayer =
                        require("game.guild.guildFuben.GuildFubenInfoLayer").new(
                        {
                            itemData = itemData,
                            showType = showType,
                            rtnObj = rtnObj,
