        ...
        levelupBtn:registerControlEventHandler(
            function(eventName, sender)
                ...
                if guildMgr:checkIsReachMaxLevel(self._buildType, self._level) == true then
                    ...
                    local msgBox =
                        require("game.guild.GuildBuildLevelUpMsgBox").new(
                        {
...
function GuildQinglongLayer:createAllLbl()
    ...
    -- 可开启挑战
    self:createShadowLbl("可开启挑战", cc.c3b(22, 255, 255), self._rootnode["open_msg_lbl"], 24)

    -- 1级开启建筑功能
    local lbl = self:createShadowLbl("1级开启建筑功能", cc.c3b(22, 255, 255), self._rootnode["open_need_msg_lbl"], 28)
