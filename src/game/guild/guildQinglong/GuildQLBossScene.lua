                    ...
                    if self._bEnd then
                        ...
                        if attackWaitTime > 0 then
                            ...
                            if self._isAutoBattle then
                                ...
                                -- 战斗过程
                                push_scene(
                                    require("game.Worldboss.WorldBossBattleScene").new(
                                        {
                                            fubenType = GUILD_QLBOSS_FUBEN,
                                            data = data,
                                            resultFunc = function()
                                                game.runningScene:addChild(
                                                    require("game.Worldboss.WorldBossBattleResultLayer").new(
                                                        {
                                                            data = data,
        ...
        GameRequest.Guild.bossResult(
            {
                ...
                callback = function(data)
                    ...
                    if data.err ~= "" then
                        ...
                        if self._bEnd then
                            self:addChild(
                                require("game.guild.guildQinglong.GuildQLBossEndResultLayer").new(
                                    {
                                        data = data,
                                        confirmFunc = function()
...
function GuildQLBossScene:ctor(param)
    ...
    self._bossSprite:setSpriteFrame(ResMgr.getHeroFrame(4902, 0))
    ...
    shuchuBtn:registerControlEventHandler(
        function(eventName, sender)
            ...
            local function toLayer(data)
                local layer =
                    require("game.guild.guildQinglong.GuildQLBossRankLayer").new(
                    {
