...
local GuildMemberVerifyItem =
    class(
    "GuildMemberVerifyItem",
    function()
...
function GuildMemberVerifyItem:create(param)
    ...
    -- 在线时间 (颜色根据在线、离线改变)
    self._onlineLbl = self:createTTF(tostring(itemData.rank), cc.c3b(58, 209, 73), self._rootnode["online_lbl"], SMALL_FONT_SIZE)
...
function GuildMemberVerifyItem:refreshItem(itemData)
    ...
    -- 战斗力
    self:createTTF(tostring(itemData.attack), cc.c3b(234, 62, 43), self._rootnode["power_lbl"])

    -- 申请时间
    self:createTTF(tostring(itemData.timeStr), cc.c3b(78, 255, 0), self._rootnode["time_lbl"])
