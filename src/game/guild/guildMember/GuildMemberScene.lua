    ...
    elseif self._verifySortType == SORT_TYPE.time then
        ...
        local function getItemByTime(time)
            ...
            for i, v in ipairs(self._verifyListData) do
                if self:getIsHasAdd(i, indexList) == false and v.createTime >= maxTime then
        ...
        return item:create(
            {
                ...
                jobFunc = function(cell)
                    ...
                    local layer =
                        require("game.guild.guildMember.GuildMemberJobLayer").new(
                        {
                            itemData = itemData,
                            title = "帮务",
                            parentScene = self
