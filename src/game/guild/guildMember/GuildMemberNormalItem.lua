...
local GuildMemberNormalItem =
    class(
    "GuildMemberNormalItem",
    function()
...
function GuildMemberNormalItem:create(param)
    ...
    -- 在线时间 (颜色根据在线、离线改变)
    self._onlineLbl = self:createTTF("待定", cc.c3b(58, 209, 73), self._rootnode["online_lbl"], SMALL_FONT_SIZE)
    self._onlineLbl:setPosition(-self._onlineLbl:getContentSize().width / 2, -self._onlineLbl:getContentSize().height / 2)

    -- 切磋次数
    self._battleTimesLbl = self:createTTF("今日剩余切磋次数:20", cc.c3b(255, 178, 69), self._rootnode["battle_times_lbl"], SMALL_FONT_SIZE)
    ...
    -- 捐献
    self._buildLbl = self:createTTF("待定", cc.c3b(255, 178, 69), self._rootnode["build_lbl"], SMALL_FONT_SIZE)
...
function GuildMemberNormalItem:refreshItem(itemData)
    ...
    if itemData.isOnline == true then
        ...
        self._onlineLbl:setColor(cc.c3b(255, 255, 255))
    end
