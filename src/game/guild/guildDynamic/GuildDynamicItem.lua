...
function GuildDynamicItem:getContentSize()
    if self._contentSz == nil then
        ...
        local rootnode = {}
        local node = CCBReaderLoad("guild/guild_guildDynamic_item.ccbi", proxy, rootnode)
...
function GuildDynamicItem:create(param)
    ...
    local proxy = CCBProxy:create()
    self._rootnode = {}
    local node = CCBReaderLoad("guild/guild_guildDynamic_item.ccbi", proxy, self._rootnode)
