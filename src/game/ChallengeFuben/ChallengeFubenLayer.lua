...
function ChallengeFubenLayer:ctor(param)
    ...
    self._rootnode = {}
    local node = CCBReaderLoad("ccbi/challenge/challengeFuben_layer.ccbi", proxy, self._rootnode, self, cc.size(display.width, height))
        ...
        buyBtn:registerControlEventHandler(
            function(eventName, sender)
                if HuoDongFuBenModel.getRestNum(self._fbId) > 0 then
                    ...
                    local buyMsgBox =
                        require("game.Challenge.HuoDongBuyMsgBox").new(
                        {
                            aid = self._fbId,
                            closeFunc = function()
    ...
    checkBtn:registerControlEventHandler(
        function(eventName, sender)
            ...
            GameAudio.playSound(ResMgr.getSFX(SFX_NAME.u_queding))
            local layer =
                require("game.ChallengeFuben.ChallengeFubenRewardLayer").new(
                {
    ...
    buzhenBtn:registerControlEventHandler(
        function(eventName, sender)
            ...
            GameAudio.playSound(ResMgr.getSFX(SFX_NAME.u_queding))
            push_scene(
                require("game.ChallengeFuben.ChallengeFubenChooseHeroScene").new(
                    {
                        fbId = self._fbId,
                        sysId = fbInfo.sys_id,
                        cards = self._cards,
        ...
        return item:create(
            {
                ...
                challengFunc = function(cell)
                    ...
                    if #self._formHero < 6 then
                        local tipLayer =
                            require("game.huashan.HuaShanHeroLessTip").new(
                            {
                                listener = function()
