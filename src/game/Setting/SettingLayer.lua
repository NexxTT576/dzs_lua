...
function SettingLayer:ctor()
    ...
    local node = CCBReaderLoad("mainmenu/setting_layer.ccbi", proxy, rootnode)
    ...
    cdkeyBtn:registerControlEventHandler(
        function(eventName, sender)
            ...
            local cdkeyRewardLayer =
                require("game.Huodong.CDKeyReward.CDKeyRewardLayer").new(
                {
                    endFunc = function()
    ...
    rootnode["returnLoginBtn"]:registerControlEventHandler(
        function(eventName, sender)
            ...
            if (device.platform == "android") then
                if
                    CSDKShell.GetSDKTYPE() == SDKType.ANDROID_YW or CSDKShell.GetSDKTYPE() == SDKType.ANDROID_WDJ or CSDKShell.GetSDKTYPE() == SDKType.ANDROID_360 or
