...
function JingYingBuyMsgBox:previewInit()
    ...
    local goldNum = ResMgr.createShadowMsgTTF({text = JingYingModel.getCost() .. "元宝", color = cc.c3b(255, 210, 0)}) --10元宝
    rowOneTable[#rowOneTable + 1] = goldNum
    local buyOne = ResMgr.createNomarlMsgTTF({text = "购买一次"})
    --购买一次
    rowOneTable[#rowOneTable + 1] = buyOne
    local fubenName = ResMgr.createShadowMsgTTF({text = "精英副本", color = cc.c3b(255, 210, 0)})
    ----某副本
    ...
    local buyNume = ResMgr.createShadowMsgTTF({text = JingYingModel.getBuyCnt(), color = cc.c3b(58, 209, 73)})
    ...
    local ableTime = ResMgr.createShadowMsgTTF({text = JingYingModel.getLimit() - JingYingModel.getBuyCnt(), color = cc.c3b(58, 209, 73)})
    --n
