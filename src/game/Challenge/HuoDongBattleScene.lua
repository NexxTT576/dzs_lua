...
function HuoDongBattleScene:jieFuResult(data)
    ...
    local resultLayer =
        require("game.Huodong.jieFuJiPinResult").new(
        {
...
function HuoDongBattleScene:updateTotalDamgeNum(num)
    if type(num) == "number" then
        ...
        self.curDamageNum:setPosition(
            self._numrootnode["cur_damage"]:getContentSize().width + self.curDamageNum:getContentSize().width / 2,
            self._numrootnode["cur_damage"]:getContentSize().height / 2
        )
        self:updataMoneyNum(self.totoNumValue)
    else
        assert(false, "错误类型")
