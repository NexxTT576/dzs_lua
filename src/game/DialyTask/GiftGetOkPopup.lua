...
function GiftGetOkPopup:setUpView()
    ...
    --关闭按钮
    cc.ui.UIPushButton.new(btnCloseRes):onButtonClicked(
        function()
            self:removeFromParent()
            GameAudio.playSound(ResMgr.getSFX(SFX_NAME.u_queding))
        end
    ):pos(self._mainPopupSize.width, self._mainPopupSize.height):addTo(self._mianPopup):setAnchorPoint(cc.p(1, 1))
    --title标签
    ...
    ):pos(self._innerContainerSize.width / 2, marginDown):addTo(self._innerContainer)
