...
function GiftGetItemView:setUpView()
    ...
    --15积分文字
    local marginLeft = 20
    local dislabel = ui.newTTFLabel({text = self._data.jifen .. "积分可领取", size = 20, font = FONTS_NAME.font_fzcy, align = ui.TEXT_ALIGN_LEFT}):pos(marginLeft, titleBngSize.height / 2):addTo(titleBng)
    dislabel:setAnchorPoint(cc.p(0, 0.5))

    --按钮
    local getBtn
    getBtn =
        cc.ui.UIPushButton.new(btnGetRes):onButtonClicked(
        function()
    ...
    local itemsViewBngs =
        ...
        "#heroinfo_title_bg.png",
        0,
        0,
        cc.size(containnerSize.width - self._padding.left - self._padding.right - marginRight - offset, containnerSize.height - self._padding.top - self._padding.down - titleBngHeight - marginTop)
