...
function RoadItemView:setUpView()
    ...
    --图标
    self._icon = display.newSprite("res/items/icon/" .. self._data.icon .. ".png"):pos(self._padding.left, containnerSize.height / 2):addTo(self._containner, 1)
    --边框
    ...
    ):pos(titleBngSize.width - marginRight, posPreY):addTo(titleBng)
    self._progresslabel:setAnchorPoint(cc.p(1, 0.5))
    ...
    --按钮
    self.goBtn =
        cc.ui.UIPushButton.new(btnGoRes):onButtonClicked(
        function()
            GameAudio.playSound(ResMgr.getSFX(SFX_NAME.u_queding))
            GameStateManager:ChangeState(self._data["goto"])
        end
    ):pos(containnerSize.width - self._padding.right, containnerSize.height / 2):addTo(self._containner)
    self.goBtn:setAnchorPoint(cc.p(1, 0.5))

    --已领奖
    self.getOkTag = display.newSprite("#getok.png"):pos(containnerSize.width - self._padding.right, containnerSize.height / 2 - 35):addTo(self._containner)
    ...
    self.getOkBtn:addNodeEventListener(
        cc.NODE_TOUCH_EVENT,
        function(event)
