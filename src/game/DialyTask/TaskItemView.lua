...
function TaskItemView:setUpView()
    ...
    --图标
    self._icon = display.newSprite("res/items/icon/" .. self._data.icon .. ".png"):pos(self._padding.left, containnerSize.height / 2):addTo(self._containner)
    ...
    ):pos(titleBngPosX + marginLeft + size.width + 10, dislabelPosY - dislabelSize.height):addTo(self._containner)
    self._disLabel:setAnchorPoint(cc.p(0, 1))
    self._disLabel:setVisible(self._data.missionCategory == 1)
    ...
    ):pos(titleBngPosX + marginLeft, dislabelPosY - dislabelSize.height):addTo(self._containner)
    self._disLabel:setAnchorPoint(cc.p(0, 1))
    ...
    self.closeBtn:addNodeEventListener(
        cc.NODE_TOUCH_EVENT,
        function(event)
