...
function HeroList:init(data)
    ...
    -- boardBg:setContentSize(cc.size(display.width,display.height))
                ...
                callback = function(listData)
                    ...
                    if self.isFirstDebrisList == nil then
                        ...
                        local itemList =
                            ...
                            {
                                size = self._rootnode["heroListBg"]:getContentSize(), --cc.size(boardBg:getContentSize().width, self.getCenterHeightWithSubTop()),-- numBg:getContentSize().height - 20),
        ...
        if TAB_TAG.XIAKE == tag then
            ...
            local function onHeroInfoLayer(index)
                if self.viewType == SALE_VIEW then
                    ...
                    local offset = self.heroTable:getContentOffset()
                    local layer =
                        require("game.Hero.HeroInfoLayer").new(
                        {
                            info = {
            ...
            if self.isFirstInitHeroTable == nil then
                ...
                self.heroTable =
                    require("utility.TableViewExt").new(
                    {
                        size = self._rootnode["heroListBg"]:getContentSize(), --cc.size(self:getContentSize().width, self.getCenterHeightWithSubTop()),-- numBg:getContentSize().height - 20),
                        direction = kCCScrollViewDirectionVertical,
                        createFunc = createFunc,
                        refreshFunc = refreshFunc,
...
function HeroList:ctor(tag)
    ...
    self._rootnode["expandBtn"]:registerControlEventHandler(
        function(eventName, sender)
            ...
            if self._cost[1] ~= -1 then
                local layer =
                    require("utility.CostTipMsgBox").new(
                    {
    ...
    self._rootnode["sellStarBtn"]:registerControlEventHandler(
        function(eventName, sender)
            ...
            local heroQuickSel =
                require("game.Hero.HeroQuickChose").new(
                quickChoseFunc,
                function()
