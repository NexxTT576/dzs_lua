...
function HeroQiangHuaLayer:updateQiangHua(param)
    ...
    if self.costNumWithShadow == nil then
        ...
        self.costNumWithShadow:setPosition(
            ...
            self._rootnode["exp_label"]:getContentSize().height * 0.6
        )
    ...
    if param.op == 2 then --如果是2的话表示强化完了 把这些值设为0
        ...
        for i = 1, #addStates do
            ...
            if addStates[i] ~= 0 then
                ...
                local fadeTime = 1
                self._rootnode["addState" .. i]:runAction(
...
function HeroQiangHuaLayer:ctor(param)
    ...
    for i = 1, 5 do
        ...
        iconBtn:addNodeEventListener(
            ...
            function(tag)
                ...
                local qiangHuaChoseLayer =
                    require("game.Hero.HeroChoseLayer").new(
                    {
    ...
    self.backBtn:addHandleOfControlEvent(
    ...
    self._rootnode["xiahun_back_btn"]:addHandleOfControlEvent(
        function(eventName, sender)
            GameAudio.playSound(ResMgr.getSFX(SFX_NAME.u_guanbi))
            for i = 1, #self.sellAbleList do
                self.sellAbleList[i]["isChosen"] = false
    ...
    self.qianghuaBtn:addHandleOfControlEvent(
        function(eventName, sender)
    ...
    self.autoBtn:addHandleOfControlEvent(
        function(eventName, sender)
            GameAudio.playSound(ResMgr.getSFX(SFX_NAME.u_queding))
    ...
    self._rootnode["xiahun_qianghua_btn"]:addHandleOfControlEvent(
        function(eventName, sender)
            GameAudio.playSound(ResMgr.getSFX(SFX_NAME.u_queding))
    ...
    self._rootnode["xiahun_5_time_btn"]:addHandleOfControlEvent(
        function(eventName, sender)
            GameAudio.playSound(ResMgr.getSFX(SFX_NAME.u_queding))
            if ResMgr.isEnoughSilver(self.xiahunCostNum) then --判断银币是否足够
                if self.curXiaHunNum >= self.needXiaHunNum then
