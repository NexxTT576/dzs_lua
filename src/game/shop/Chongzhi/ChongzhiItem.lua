...
function ChongzhiItem:getContentSize()
    if self._cntSize == nil then
        ...
        local rootnode = {}
        local node = CCBReaderLoad("ccbi/shop/shop_chongzhi_item.ccbi", proxy, rootnode)
...
function ChongzhiItem:create(param)
    ...
    local proxy = CCBProxy:create()
    self._rootnode = {}
    local node = CCBReaderLoad("ccbi/shop/shop_chongzhi_item.ccbi", proxy, self._rootnode)
