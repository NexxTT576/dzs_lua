    ...
    function(param)
        ...
        if param.type then
            if param.type == 1 then
                node = CCBReaderLoad("login/login_server_name_1.ccbi", proxy, rootnode)
            else
                node = CCBReaderLoad("login/login_server_name_2.ccbi", proxy, rootnode)
            end
        else
            node = CCBReaderLoad("login/login_server_name.ccbi", proxy, rootnode)
        end
...
function ServerChooseLayer:ctor(serverList, callback)
    ...
    local proxy = CCBProxy:create()
    self._rootnode = {}
    local contentNode = CCBReaderLoad("login/login_server_choose.ccbi", proxy, self._rootnode)
    ...
    self._rootnode["serverNameNode"]:addNodeEventListener(
