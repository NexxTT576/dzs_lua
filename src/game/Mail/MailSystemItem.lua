...
function MailSystemItem:getContentSize()
    if self._contentSz == nil then
        ...
        local rootnode = {}
        local node = CCBReaderLoad("mail/mail_system_item.ccbi", proxy, rootnode)
...
function MailSystemItem:create(param)
    ...
    local proxy = CCBProxy:create()
    self._rootnode = {}
    local node = CCBReaderLoad("mail/mail_system_item.ccbi", proxy, self._rootnode)
