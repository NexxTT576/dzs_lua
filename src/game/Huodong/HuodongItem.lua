...
function HuodongItem:getContentSize()
    if self._sz then
        ...
        local node = CCBReaderLoad("ccbi/huodong/ccb_huodong_item.ccbi", proxy, rootnode)
...
function HuodongItem:create(param)
    local proxy = CCBProxy:create()
    self._rootnode = {}
    local node = CCBReaderLoad("ccbi/huodong/ccb_huodong_item.ccbi", proxy, self._rootnode)
    self:addChild(node)
