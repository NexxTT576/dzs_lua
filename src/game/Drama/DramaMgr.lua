...
function DramaMgr.createChoseLayer(data)
    ...
    if SHOW_DRAMA then
        ...
        app:changeState(GAME_STATE.DRAMA_SCENE, msg)
    else
        dramaEndStartLogin()
    end
end
    ...
    TutoMgr.getServerNum(
        function(plotNum)
            if plotNum == 0 and game.player.m_level == 1 then
                app:changeState(GAME_STATE.STATE_NORMAL_BATTLE, {levelData = {id = 110101}, grade = 1, star = 0})
            else
                if plotNum == 0 then
                ...
                local msg = {}
                msg.showNote = true
                app:changeState(GAME_STATE.STATE_MAIN_MENU, msg)
            end
        end
    )
end
