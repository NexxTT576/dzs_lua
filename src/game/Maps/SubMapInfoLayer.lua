...
local SubMapInfoLayer =
    class(
    "SubMapInfoLayer",
    function(levelData, _subMapInfo)
...
function SubMapInfoLayer:createBuyMsgBox()
    ...
    local buySubMapBox =
        require("game.Maps.SubMapBuyMsgBox").new(
        {
...
function SubMapInfoLayer:normalFight(grade, levelData, isPassed)
    ...
    if game.player:getStrength() < self._needPower then
        self:addChild(
            require("game.Maps.TiliMsgBox").new(
                {
...
function SubMapInfoLayer:lianzhanFight(grade)
    ...
    if self._secWait > 0 then
        local msgBox =
            require("game.Maps.LianzhanMsgBox").new(
            {
            ...
            RequestHelper.lianzhan.battle(
                {
                    ...
                    callback = function(data)
                        ...
                        if string.len(data["0"]) > 0 then
                            ...
                            self._hastouchLianzhan = false
                            -- 连战结果
                            local lianzhanResult =
                                require("game.Maps.LianzhanLayer").new(
                                {
                                    id = self._id,
                                    data = data
                                }
