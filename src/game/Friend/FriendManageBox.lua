...
function FriendManageBox:createBreakBox()
    local rowOneTable = {}
    local descFirst = ResMgr.createShadowMsgTTF({text = "你确定要删除好友", color = cc.c3b(255, 255, 255)})
    rowOneTable[#rowOneTable + 1] = descFirst
    local name = ResMgr.createShadowMsgTTF({text = self.playerName, color = cc.c3b(255, 210, 0)})
    rowOneTable[#rowOneTable + 1] = name
    local descEnd = ResMgr.createShadowMsgTTF({text = "吗？", color = cc.c3b(255, 255, 255)})
    rowOneTable[#rowOneTable + 1] = descEnd

    local rowAll = {rowOneTable}

    local curAcc = self.acc
    local friendBreakBox =
