...
function FriendCell:initBtnEvent()
    if self.cellType ~= 4 then
        ResMgr.setControlBtnEvent(
            self._rootnode["send_naili"],
            function()
                self:onSendNaili()
            end
        )
        ResMgr.setControlBtnEvent(
            self._rootnode["apply_btn"],
            function()
                self:onApply()
            end
        )
        ResMgr.setControlBtnEvent(
            self._rootnode["get_naili"],
            function()
                self:onGetNaili()
            end
        )
        ...
        ResMgr.setControlBtnEvent(
            self._rootnode["agree_btn"],
            function()
                self:onAgree()
            end
        )
        ResMgr.setControlBtnEvent(
            self._rootnode["reject_btn"],
            function()
                self:onReject()
            end
        )
...
function FriendCell:onApply()
    if game.player:checkIsSelfByAcc(self.account) then
        ...
        local applyBox =
            require("game.Friend.FriendApplyBox").new(
            {
