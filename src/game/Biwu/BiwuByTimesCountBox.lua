...
function BiwuByTimesCountBox:ctor(param, successCallBack)
    ...
    local function getCost()
        ...
        elseif tmpNum < param.maxN + 2 then
            ...
            costNum =
                (param.maxN - param.hadBuy + 1) * (param.baseprice + param.hadBuy * param.addPrice) + (param.maxN - param.hadBuy + 1) * (param.maxN - param.hadBuy) * param.addPrice / 2 +
                (tmpNum - param.maxN - 1) * (param.baseprice + param.maxN * param.addPrice)
        end
        return costNum
    end
