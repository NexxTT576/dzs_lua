...
function WorldBossRankItem:getContentSize()
    if self.cntSize == nil then
        ...
        CCBReaderLoad("huodong/worldBoss_rank_item.ccbi", proxy, rootNode)
...
function WorldBossRankItem:create(param)
    ...
    self._rootnode = {}
    local proxy = CCBProxy:create()
    local node = CCBReaderLoad("huodong/worldBoss_rank_item.ccbi", proxy, self._rootnode)
