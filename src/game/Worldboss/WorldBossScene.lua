        ...
        GameRequest.worldBoss.result(
            {
                callback = function(data)
                    ...
                    if data["0"] ~= "" then
                        ...
                        if self._bEnd then
                            self:addChild(
                                require("game.Worldboss.WorldBossEndResultLayer").new(
                                    {
                                        data = data,
                                        confirmFunc = function()
...
function WorldBossScene:ctor()
    ...
    self._rootnode["shuchuBtn"]:addHandleOfControlEvent(
        function(eventName, sender)
            GameAudio.playSound(ResMgr.getSFX(SFX_NAME.u_queding))
    ...
    self._rootnode["extraRewardBtn"]:addHandleOfControlEvent(
        function(eventName, sender)
            GameAudio.playSound(ResMgr.getSFX(SFX_NAME.u_queding))
