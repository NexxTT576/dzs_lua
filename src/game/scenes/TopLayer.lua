...
function TopLayer:ctor(isOther)
    ...
    if isOther ~= nil and isOther == true then
        node = CCBReaderLoad("public/top_frame_other.ccbi", proxy, self._rootnode)
    else
        node = CCBReaderLoad("public/top_frame.ccbi", proxy, self._rootnode)
    end

    node:setPosition(display.width / 2, display.height)
    self:addChild(node)
