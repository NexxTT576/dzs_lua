...
function SkillQiangHuaLayer:ctor(param)
    ...
    for k, v in ipairs(game.player:getSkills()) do
        if self._info._id ~= v._id and v.cid == 0 then
            if (data_item_item[v.resId].pos == (96 + _pos)) or (_pos == data_item_item[v.resId].pos and data_item_item[v.resId].lysis and data_item_item[v.resId].lysis == 1) then
                table.insert(
                    self._listData,
                    {
    ...
    local function onChooseLayer(tag, sender)
        ...
        push_scene(
            require("game.skill.SkillChooseScene").new(
                {
        ...
        for k, v in ipairs(unlockStates.arr_level) do
            ...
            if v <= data_shangxiansheding_shangxiansheding[8].level then
                if self._info.lv < v then
                    str = str .. "(" .. v .. "级解锁)"
                    extraState:setColor(cc.c3b(137, 137, 137))
                else
                    extraState:setColor(cc.c3b(147, 5, 0))
                end
...
function SkillQiangHuaLayer:refreshIcon()
    ...
    for k, v in pairs(self._selected) do
        if v then
            ...
            cost = cost + self._listData[k].data.curExp + data_kongfu_kongfu[self._listData[k].data.level + 1]["sumexp"][self._listData[k].baseData.quality] + self._listData[k].baseData.exp
        end
    end
