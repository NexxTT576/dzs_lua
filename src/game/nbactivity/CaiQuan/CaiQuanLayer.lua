...
function CaiQuanLayer:onMainView(data)
    ...
    if self.isFirstShow then
        ...
        for i = 1, #self.caiQuanModel.itemList do
            ...
            self._rootnode["item_icon_" .. i]:addNodeEventListener(
                cc.NODE_TOUCH_EVENT,
                function(event)
                    local itemInfo =
    ...
    RequestHelper.getGuessInfo(
        {
            callback = function(data)
                ...
                self:timeSchedule(
                    {
                        time = self.caiQuanModel.endTime,
                        label = self.endTimeLabel,
                        callBack = function()
                            self.isTimeOut = true
                            print("time out")
...
function CaiQuanLayer:ctor(param)
    ...
    -- 创建UI
    local contentNode = CCBReaderLoad("nbhuodong/caiquan_layer.ccbi", proxy, self._rootnode, self, viewSize)
