...
function DuiHuanGiftPopup:setUpTableView()
    local titleDis =
        CCLabelTTF:create(require("data.data_message_message")[22].text, FONTS_NAME.font_fzcy, 20, cc.size(self._innerContainerSize.width - 40, 0), kCCTextAlignmentLeft, kCCVerticalTextAlignmentTop)
    ...
    self._tableContainer =
        display.newSprite(commonRes.tableViewBngRes, 0, 0, {scale9 = true, size = cc.size(self._innerContainerSize.width - self._tableContainerBorderOffset * 2, self._innerContainerSize.height - 50)}):pos(
