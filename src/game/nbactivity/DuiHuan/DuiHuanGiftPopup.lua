...
function DuiHuanGiftPopup:setUpView()
    ...
    self._mianPopup = display.newScale9Sprite(commonRes.mainFrameRes, 0, 0, 
                    cc.size(display.width - self._mainFrameWidthOffset,
                        display.height - 100))
    ...
    --title标签
    self._titleDisLabel = ui.newBMFontLabel({text = "兑换预览", size = self._titleDisFontSize, 
        align = ui.TEXT_ALIGN_CENTE,font = "res/fonts/font_title.fnt"})
        :pos( self._mainPopupSize.width / 2, self._mainPopupSize.height - self._titleDisOffsetOfTop)
        :addTo(self._mianPopup)
...
function DuiHuanGiftPopup:setUpTableView()
	local titleDis = CCLabelTTF:create(require("data.data_message_message")[22].text, FONTS_NAME.font_fzcy, 20,
        								cc.size(self._innerContainerSize.width - 40,0),
	...
	titleDis:setColor(ccc3(99,47,8))
    ...
    self._tableContainer = display.newScale9Sprite(commonRes.tableViewBngRes, 0, 0, 
                        ...
                        self._innerContainerSize.height - 50))
...
function DuiHuanGiftPopup:tableCellTouched(table,cell)
...
function DuiHuanGiftPopup:onIconClick(data)
	if tonumber(data[1]) ~=6 then
		...
		if not CCDirector:sharedDirector():getRunningScene():getChildByTag(1111) then
			local descLayer = require("game.Spirit.SpiritInfoLayer").new(4, {resId = tonumber(data[2])},nil,closeFunc)
			CCDirector:sharedDirector():getRunningScene():addChild(descLayer, 100000000,1111)
		end
	end
end

function DuiHuanGiftPopup:cellSizeForTable(table,idx)
...
function DuiHuanGiftPopup:tableCellAtIndex(table, idx) 
    local cell = CCTableViewCell:new()
    local count = #(string.split(self._data[idx + 1].view,";")) / 2
    local itemView = GiftGetItemView.new(cc.size(self._tableContainerSize.width, 60 + math.ceil(count / 4) * (95 + 35)), 
        self._data[idx + 1] ,self._mainMenuScene,self)
