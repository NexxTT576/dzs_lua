...
function ActivityItemView:setUpView()
                ...
                RequestHelper.dialyTask.getTaskGift(
                    {
                        ...
                        callback = function(data)
                            ...
                            if data["0"] ~= "" then
                                ...
                                display.newSprite("#getok.png"):pos(getBtn:getPosition()):addTo(itemsViewBngs):setAnchorPoint(cc.p(1, 0.5))
    ...
    local getTag = display.newSprite("#getok.png"):pos(getBtn:getPosition()):addTo(itemsViewBngs)
    getTag:setAnchorPoint(cc.p(1, 0.5))

    getBtn:setVisible(self._data.state ~= 3)
    if self._data.state == 1 then
        getBtn:setSpriteFrame(display.newSprite(btnGetRes.disabled):getSpriteFrame())
        getBtn:setTouchEnabled(false)
    elseif self._data.state == 2 then
        getBtn:setSpriteFrame(display.newSprite(btnGetRes.normal):getSpriteFrame())
