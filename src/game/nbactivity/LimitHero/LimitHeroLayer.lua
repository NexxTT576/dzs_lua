...
function LimitHeroLayer:init()
    ...
    self.touchLayer:addNodeEventListener(
        cc.NODE_TOUCH_EVENT,
        function(event)
    ...
    timePeriodTTF:setPosition(
        self._rootnode["hero_ttf"]:getPositionX(),
        self._rootnode["hero_ttf"]:getPositionY() + self._rootnode["hero_ttf"]:getContentSize().height / 2 + timePeriodTTF:getContentSize().height / 2
    )
    ...
    self.freeDrawLabel:setPosition(
        self._rootnode["free_btn"]:getPositionX(),
        self._rootnode["free_btn"]:getPositionY() - self._rootnode["free_btn"]:getContentSize().height / 2 - self.freeDrawLabel:getContentSize().height / 2
    )
...
function LimitHeroLayer:updateRightDesc()
    ...
    local ttfColor = {cc.c3b(255, 210, 0), cc.c3b(36, 255, 0), cc.c3b(255, 255, 255)}
    local textContent = {
        "排名",
        {" 第1", " 2-3", " 4-20", " 21-50"},
        " 可获得"
...
function LimitHeroLayer:createDrawSuccessLayer(param)
    ...
    local zhaojiangLayer =
        require("game.shop.ZhaojiangResultNormal").new(
        {
            type = 4,
            herolist = herolist,
