...
function VipFuliLayer:ctor(param)
    ...
    local node = CCBReaderLoad("nbhuodong/vipFuli_layer.ccbi", proxy, self._rootnode, self, viewSize)
    self:addChild(node)
...
function VipFuliLayer:initData(data)
    ...
    checkVipBtn:addHandleOfControlEvent(
        function(eventName, sender)
            ...
            local vipInfoLayer =
                require("game.shop.Chongzhi.ChongzhiVipDesInfoLayer").new(
                {
    ...
    -- 领取奖励按钮
    self._rootnode["getRewardBtn"]:addHandleOfControlEvent(
        function(eventName, sender)
            GameAudio.playSound(ResMgr.getSFX(SFX_NAME.u_queding))
