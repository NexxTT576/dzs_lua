...
function EquipQiangHuaLayer:ctor(param)
    ...
    for i = 1, 5 do
        if i > #self.stateStrs then
            ...
            self._rootnode["arrow_" .. i]:setPosition(
                self._rootnode["state_num_" .. i]:getPositionX() + self._rootnode["state_num_" .. i]:getContentSize().width + 10 + arrowLen / 2,
                self._rootnode["state_num_" .. i]:getPositionY()
            )
            self._rootnode["state_add_" .. i]:setPosition(
                self._rootnode["state_num_" .. i]:getPositionX() + self._rootnode["state_num_" .. i]:getContentSize().width + 10 + arrowLen,
                self._rootnode["state_num_" .. i]:getPositionY()
            )
        end
    end
...
function EquipQiangHuaLayer:qiangHuaUpdate(subData)
    ...
    for index = 1, #self.stateStrs do
        ...
        self._rootnode["arrow_" .. index]:setPosition(
            self._rootnode["state_num_" .. index]:getPositionX() + 10 + self._rootnode["state_num_" .. index]:getContentSize().width + arrowLen / 2,
            self._rootnode["state_num_" .. index]:getPositionY()
        )
        self._rootnode["state_add_" .. index]:setPosition(
            self._rootnode["state_num_" .. index]:getPositionX() + 10 + self._rootnode["state_num_" .. index]:getContentSize().width + arrowLen,
            self._rootnode["state_num_" .. index]:getPositionY()
        )
    end
