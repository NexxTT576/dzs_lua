...
function UrgencyBroadcast:ctor()
    ...
    local rootnode = {}
    local node = CCBReaderLoad("public/urgency_broadcast.ccbi", proxy, rootnode)
    self:addChild(node)
    ...
    --@RefType luaIde#cc.DrawNode
    local stencil = cc.DrawNode:create()
    stencil:drawRect(cc.p(0, 0), cc.p(self._clippingNode:getContentSize().width, self._clippingNode:getContentSize().height), cc.c4f(1, 1, 1, 1))
